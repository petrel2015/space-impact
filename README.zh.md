...
- ⚙️ **设置在右上角导航** —— 难度/主题/音效/自动连发/瞄准虚线随时可调（游戏中打开会自动暂停）。
...
实用 URL 参数（测试 / 分享链接）：`?lang=zh|en` · `?theme=retro|night|paper` · `?touch=1` · `?autostart=1` · `?demo=1`（自动驾驶，配合 autostart）· `?paused=1` · `?aim=0|1` · `?level=14` ——[完整参数表](docs/zh/usage.md#url-参数)。
