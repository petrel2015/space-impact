...
### 变更
- 战役由 14 关重切为五集单元剧**《归途》**：一关一个星域、敌人生态
  严格锁定（碎石回廊 / 猩红虫巢 / 钢铁坟场 / 掠夺者封锁线 / 时空
  乱流——第 5 关按波次分段混合，段间留喘息），第 4 关有「先锋」
  「哨卫」两波中 Boss 轮值；每关专属场景配色与可跳过的开场/通关
  旁白。Boss 召唤物对齐生态（蜂后产卵放幼蜂、防御主脑放巡逻机）。
  旧 6-14 关存档按进度比例映射到新战役（旧第 8 关 ≈ 新第 3 关）；
  五个二周目变体 Boss 保留在数据包中不删。图鉴卡片新增「栖息地」
  一行，标注每只怪物的星域归属。
